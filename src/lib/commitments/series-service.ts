...
export async function listSeries(ownerId: string): Promise<SeriesView[]> {
  await connectToDatabase();
  const rows = await SeriesModel.find({ ownerId }).sort({ createdAt: -1 }).lean();
...
export async function updateSeries(
  ...
  const series = await SeriesModel.findOne({ _id: seriesId, ownerId }).lean();
  ...
  if (data.scope === 'this-occurrence') {
    ...
    await CommitmentModel.updateOne({ _id: occurrence._id, ownerId }, { $set: changes });
    await appendEvent({
...
export async function endSeries(
  ...
  await SeriesModel.updateOne(
    { _id: seriesId, ownerId },
    { $set: { status: 'ended', endDate: today } },
  );
