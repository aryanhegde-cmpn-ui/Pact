...
vi.mock('@/lib/db/models/notification', () => ({
  NotificationModel: {
    find: (query: { ownerId: string; status: string; scheduledFor: { $lte: Date } }) => ({
      sort: () => ({
        limit: () => ({
          lean: async () =>
            store.notifications
    ...
    updateMany: async (
      filter: { _id: { $in: unknown[] }; ownerId?: string },
...
beforeEach(() => {
  store.notifications = [];
  store.commitments = [{ _id: 'c1', ownerId: OWNER, status: 'pending' }];
});
...
describe('deliverDue', () => {
  it('sends a notification that is due and fresh', async () => {
    ...
    const report = await deliverDue(OWNER, NOW);
  ...
  it('skips anything whose commitment is already resolved', async () => {
    store.commitments = [{ _id: 'c1', ownerId: OWNER, status: 'done' }];
    queue({ scheduledFor: new Date(NOW.getTime() - 60_000) });
  ...
  it('is a no-op when nothing is due', async () => {
    expect(await deliverDue(OWNER, NOW)).toEqual({ sent: 0, skippedStale: 0, skippedResolved: 0 });
  });

  it('does not re-send on a second pass', async () => {
    queue({ scheduledFor: new Date(NOW.getTime() - 60_000) });

    await deliverDue(OWNER, NOW);
    const second = await deliverDue(OWNER, new Date(NOW.getTime() + 60_000));
